...
// Summary holds the counts and findings main accumulates while it works,
// so they can all be printed together at the end.
type Summary struct {
	Target          string
	DryRun          bool
	AttackerIPs     []string
...
// Run re-checks that no matching process and no matching registry value
// remain, then prints the run's summary. It returns true if the system is
// clean. In dry-run mode nothing was actually removed, so the checks are
// skipped and the summary is printed as a preview.
...
// printSummary prints what the run found and did, independent of whether
// verification passed.
